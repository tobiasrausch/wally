...
namespace wallysworld
{
  ...
  inline int matchRun(TConfigStruct& c) {
    ...
    for(TReadMappings::iterator it = mp.begin(); it != mp.end(); ++it) {
      ...
      // Debug
      for(uint32_t i = 0; i < it->second.size(); ++i) {
	std::cerr << hdr->target_name[it->second[i].tid] << ':' << it->second[i].gstart << '-' << it->second[i].gend << '\t' << it->second[i].rstart << '-' << it->second[i].rend << '(' << (int) it->second[i].fwd << ')' << '\t' << it->first << std::endl;
      }
    }
