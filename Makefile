...
ifeq (${STATIC}, 1)
	LDFLAGS += `pkg-config --cflags --libs --static opencv4` -static -static-libgcc -pthread -lhts -lz -llzma -lbz2
else
	LDFLAGS += `pkg-config --cflags --libs opencv4` -pthread -lhts -lz -llzma -lbz2 -Wl,-rpath,${EBROOTHTSLIB}
endif
