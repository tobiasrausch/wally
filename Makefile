...
# Wasm
wasm: src/wally client/src/static/js/wally.js
...
client/src/static/js/wally.js: src/wasm.cpp ${EMCONF} ${WASMHTS} ${WASMB2D} ${WASMBOOST}
	${EMCXX} -O2 -std=c++17 -fexceptions -DNDEBUG -DBOOST_FILESYSTEM_NO_CXX20_ATOMIC_REF -I ${WASMDIR}/htslib -I ${BLEND2DSRC} -I ${WASMDIR}/boost src/wasm.cpp ${WASMHTS} ${WASMB2D} ${WASMBOOST} -sUSE_ZLIB=1 -sALLOW_MEMORY_GROWTH=1 -sINITIAL_MEMORY=64MB -sMAXIMUM_MEMORY=2GB -sENVIRONMENT=web,worker -sMODULARIZE=1 -sEXPORT_ES6=1 -sEXPORT_NAME=createWally -sFORCE_FILESYSTEM=1 -sEXPORTED_FUNCTIONS=_wally_region,_wally_slice,_wally_dotplot,_malloc,_free -sEXPORTED_RUNTIME_METHODS=FS,ccall,cwrap -lworkerfs.js -o client/src/static/js/wally.js

wasm-clean:
	rm -rf ${WASMDIR}/htslib ${WASMDIR}/boost ${WASMDIR}/emcache ${EMCONF} ${BLEND2DSRC}/build-wasm client/src/static/js/wally.js client/src/static/js/wally.wasm
